		...
		//==> jQuery 적용 추가된 부분
		 $(function() {
			...
			//==> DOM Object GET 3가지 방법 ==> 1. $(tagName) : 2.(#id) : 3.$(.className)
		 	$( ".Depth03:contains('개인정보조회')" ).css('color','blue').on("click" , function() {
				//Debug..
			...
			//==> 회원정보조회 Event 연결처리부분
			//==> DOM Object GET 3가지 방법 ==> 1. $(tagName) : 2.(#id) : 3.$(.className)
		 	$( ".Depth03:contains('회원정보조회')" ).css('color','blue').on("click" , function() {
				//Debug..
				//alert(  $( ".Depth03:contains('회원정보조회')" ) );
		 		$(window.parent.frames["rightFrame"].document.location).attr("href","/user/listUser");
			}); 
			
		 	$( ".Depth03:contains('판매상품등록')" ).css('color','blue').on("click" , function() {
		 		$(window.parent.frames["rightFrame"].document.location).attr("href","../product/addProductView.jsp;");
		 	}); 
		 	
		 	$( ".Depth03:contains('판매상품관리')" ).css('color','blue').on("click" , function() {
		 		$(window.parent.frames["rightFrame"].document.location).attr("href","/Product/listProduct?menu=manage");
		 	}); 
		 	
		 	$( ".Depth03:contains('상 품 검 색')" ).css('color','blue').on("click" , function() {
		 		$(window.parent.frames["rightFrame"].document.location).attr("href","/Product/listProduct?menu=search");
		 	}); 
		 	
		 	$( ".Depth03:contains('구매목록조회')" ).css('color','blue').on("click" , function() {
		 		$(window.parent.frames["rightFrame"].document.location).attr("href","/Purchase/listSale");
		 	}); 
		 	
		 	$( ".Depth03:contains('구매이력조회')" ).on("click" , function() {
		 		$(window.parent.frames["rightFrame"].document.location).attr("href","/Purchase/listPurchase?userId=${user.userId}");		 		
		 	}).css('color','blue'); 
...
<tr>
	<td valign="top"> 
		<table  border="0" cellspacing="0" cellpadding="0" width="159">
			...
			<tr>
				<td class="Depth03">
				<!-- 	<a href="/listPurchase.do"  target="rightFrame"></a> -->
					구매이력조회
				</td>
			</tr>
			</c:if>
